...
def get_uri(uri):
    ...
    :param uri: The URI to retrieve in the form ``scheme://path``. The
        ``scheme`` may currently be anything supported by ``urllib2`` in
        addition to ``"mongodb"`` which uses the `standard connection string
        format`_ with a trailing ``"/collection-name"`` for the collection to
        retrieve. Web URIs are retrieved via a GET request.
        File paths may be relative to the current working directory by omitting
        a third slash and directly beginning the relative path.
    :returns: The data from the URI as a string.

    .. _`standard connection string format`: http://docs.mongodb.org/manual/\
