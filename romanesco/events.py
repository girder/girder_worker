"""
This module contains the romanesco events framework. Listeners should subscribe
to events by calling, e.g.

    ``romanesco.events.bind('event.name', 'my.handler', handler_function)``

And events should be published like so:

    ``e = romanesco.events.trigger('event.name', info)``
